...
{
  return re_search_2 (bufp, NULL, 0, string, size, 
startpos, range,
                      regs, 
size);
}
...
# endif
    ret = byte_regex_compile (pattern, 
...
re_comp (const char *s)
{
  ...
  if (!re_comp_buf.buffer)
    {
      re_comp_buf.buffer = (unsigned char *) malloc (
200);
...
# endif
    ret = byte_regex_compile (s, strlen (s), re_syntax_options, 
&re_comp_buf);
  if (!ret)
    return NULL;
...
# endif
    ret = byte_regex_compile (pattern, strlen (pattern), syntax, 
preg);
...
string);
  ...
  ret = re_search (&private_preg, string, 
len,
                   /* start: */ 0, /* range: */ 
len,
