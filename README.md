...
- **WSL2 with Ubuntu 22.04** (primary build environment)
- **ARM GNU Toolchain** — arm-none-eabi-gcc 13+ (installed by `make setup`)
- Python 3.8+, CMake, make (installed by `make setup`)
- Target: **RP2350B** (ARM Cortex-M33, 150 MHz)

### Quick Start (WSL2 Ubuntu)

All commands run inside a WSL2 Ubuntu terminal:

```bash
cd /mnt/c/Users/weyst/Documents/picocalc-term  # or wherever your repo lives
...
- Exclude the workspace from Windows Defender real-time scanning
